...
read -r -d '' HELP <<EOF
redirects_helper test runner. Execute unit test suite and some other details.
...
shift $((OPTIND - 1))
if [ -n "${1}" ]; then
    TEST_FILE="Web/typo3conf/ext/redirects_helper/${1}"
else
    case ${TEST_SUITE} in
        functional)
            TEST_FILE="Web/typo3conf/ext/redirects_helper/Tests/Functional"
            ;;
        unit)
            TEST_FILE="Web/typo3conf/ext/redirects_helper/Tests/Unit"
            ;;
    esac
fi
...
case ${TEST_SUITE} in
    ...
    cgl)
        ...
        if [[ ! -z ${CGLCHECK_DRY_RUN} ]]; then
            # The --diff-format has been removed in version 3.0, the format is always udiff now.
